...
<div id="page-main">
    <div id="result">        
        ...
        <?php
        $sql= "SELECT COUNT(*) FROM `everyone3` " ;
        $result = execute_sql($database, $sql, $link) ;
        $sum = mysql_result($result,0,0) ;

        //逐筆顯示
        $sql = "SELECT * FROM `everyone3` order by id ASC";
	    $result = execute_sql($database, $sql, $link) ;
        ...
        for ($i=0 ; $i<$sum ; $i++){
            ...
            switch ( $temp[0] ){
                case "d69":   $bingo_str1 = "憨兒工讀獎" ;  break ;
        ...
        $sql= "SELECT COUNT(*) FROM `everyone3` WHERE bingo LIKE '不可抽獎'" ;
        $result = execute_sql($database, $sql, $link) ;
        $bingo_no = mysql_result($result,0,0) ;

        $sql= "SELECT COUNT(*) FROM `everyone3` WHERE bingo <> '不可抽獎' AND bingo <> ''" ;
        $result = execute_sql($database, $sql, $link) ;
